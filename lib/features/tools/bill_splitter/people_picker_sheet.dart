...
class _PeoplePickerSheetState extends ConsumerState<_PeoplePickerSheet> {
  ...
  Future<void> _addNewPerson() async {
    ...
    if (name.toLowerCase() == kYouName.toLowerCase()) {
      showKuberSnackBar(
        context,
        '"You" is reserved and always included',
        isError: true,
      );
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    final allNames = [
      kYouName,
      ...people.map((p) => p.name).where((n) => n != kYouName),
    ];
...
class _AddPersonInput extends StatelessWidget {
  final TextEditingController controller;
  final VoidCallback onSubmit;
  final VoidCallback onClose;

  const _AddPersonInput({
