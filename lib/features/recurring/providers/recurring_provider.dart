...
final upcomingRecurringProvider = FutureProvider<List<RecurringRule>>((
  ref,
) async {
...
final recentlyProcessedProvider = FutureProvider<List<Transaction>>((
  ref,
) async {
  final all = await ref.watch(transactionListProvider.future);
  return all.where((t) => t.linkedRuleType == 'recurring').take(5).toList();
});
...
final recurringRuleTransactionsProvider =
    FutureProvider.family<List<Transaction>, int>((ref, ruleId) async {
      ...
      final all = await ref.watch(transactionListProvider.future);
      return all.where((t) => t.linkedRuleId == ruleId.toString()).toList()
